...
@section('page_content')
  <div class="notebook-page">
    ...
    <div class="notebook-page__main">
      ...
      <div class="item-form">
        {{ Form::model($item, ['method' => 'PUT', 'route' => ['update_item', $notebook->id, $item->id]]) }}

          <h2 class="form-heading">Edit: {{ $item->name }}</h2>

          @include('items.partials.form')

          <div class="form-block--buttons">
            {{ link_to_route('edit_item', 'CANCEL', [$notebook->id, $item->id], ['class' => 'form-button--secondary']) }}
            {{ Form::submit('UPDATE ITEM', ['class' => 'form-button']) }}
          </div>
        {{ Form::close() }}
      </div>
